...
use App\Services\OpenFoodFactsImportService;
...
class ImportOpenFoodFactsJob implements ShouldQueue
{
    ...
    {
        try {
            app(OpenFoodFactsImportService::class)->importFromGzStream($this->filename);
        } catch (\Exception $e) {
            Log::error("Erro no job de importação: " . $e->getMessage());
        }
